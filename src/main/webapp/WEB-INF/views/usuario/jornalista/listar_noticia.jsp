...
<h2>Listar Noticias</h2>
	...
	<c:forEach var="n" items="${noticias}">
		<tr>
			<td><a href="visualizarNoticia?id_noticia=${n.id_noticia}"><h1>${n.titulo}</h1><br /><h3>${n.subtitulo}</h3></a></td>
			<td>${n.secao.titulo}</td>
			<td>${usuario_logado.nome}</td>
			<td>${n.data_noticia}</td>
			<td><button class="dropbtn"><a href="apagarNoticia?id_noticia=${n.id_noticia}">APAGAR</a></button></td>
			<td><button class="dropbtn"><a href="alterarNoticiaFormulario?id_noticia=${n.id_noticia}">ALTERAR</a></button></td>
		</tr>
	</c:forEach>
	</table>	
