...
<title>Listar Jornalistas</title>
</head>
<body>
...
<h2>Listar Jornalistas</h2>
	...
	<c:forEach var="j" items="${jornalistas}">
		<tr>
			...
			<td>${j.email}</td>
			<td>${j.login}</td>
			<td><button class="dropbtn"><a href="apagarUsuario?id_usuario=${j.id_usuario}">APAGAR</a></button></td>
			<td><button class="dropbtn"><a href="alterarUsuarioFormulario?id_usuario=${j.id_usuario}">ALTERAR</a></button></td>
		</tr>
	</c:forEach>
	</table>	
